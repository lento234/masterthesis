...
Figure \ref{fig:hisc_dd} shows the domain decomposition of the hybrid simulation, performed according to section \ref{sec:dotd}. The Lagrangian domain $\Omega_L$ resolves the full fluid domain, with an Eulerian domain $\Omega_E$ resolving the near-wall region of the cylinder. The Eulerian domain $\Omega_E$ is bounded by boundary $\partial\Omega_E=\Sigma_d\cup\Sigma_{wall}$, where $\Sigma_d$ is the external Dirichlet boundary and $\Sigma_{wall}$ is the no-slip wall of the cylinder. The interpolation region $\Omega_{int}$, where we correct the particle strengths, is within the Eulerian domain $\Omega_E$, i.e $\Omega_{int}\subset\Omega_E$. The interpolation region $\Omega_{int}$ is bounded by $\partial\Omega_{int}=\Sigma_p\cup\Sigma_{int;ext}$, where $\Sigma_p$ is the vortex panel domain boundary, and $\Sigma_{int;ext}$ is the external boundary of the interpolation domain. The vortex panel domain boundary $\Sigma_p$, is defined with an offset $d_{surf}=3\cdot{h}$ from the cylinder wall $\Sigma_{wall}$. The exterior boundary $\Sigma_{int;ext}$ of the interpolation region $\Omega_{int}$ is defined with a larger offset $d_{bdry} = 0.1\cdot{R}$ for the boundary $\Sigma_d$. We observed in the previous test cases that the main error of the hybrid scheme is the artificial vorticity generated at the $\Sigma_d$ and therefore to minimize this error, we have chosen a larger offset than used by Stock \cite{Stock2010a}.

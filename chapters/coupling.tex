...
\label{sec:coupling-mlca}
	...
	\item \textbf{Setup a structured grid} (\textit{only once}): A \indexAcron{structured grid}{SG} is created in the local coordinate system of the $k^{\mathrm{th}}$ Eulerian domain $\Omega_E^k$. The grid is constructed such that it covers the \indexAcron{Eulerian grid}{SG} of the Eulerian method, as shown in Figure \ref{fig:structuredGrid_localCoord}.
